...
def collate_fn(samples: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for key in samples[0].keys():
        ...
        # ---------- 3b. Fixed-length keys, directly stack ----------
            ...
            batched[key] = torch.tensor(vals, dtype=torch.long)
...
def build_dataloader(
    ...
    pretrain_use_split = bool(getattr(args, "pretrain_use_split", False))

    if is_pretrain and pretrain_use_split:
        mode_for_dataset = mode
    elif is_pretrain:
        mode_for_dataset = "full"
    else:
        mode_for_dataset = mode
    ...
    pretrain_fold = int(getattr(args, "pretrain_split_fold", getattr(args, "fold", 0)))
    
    dataset = dataset_cls(
        pkl_path,
        fold=pretrain_fold if is_pretrain else args.fold,
        mode=mode_for_dataset,
