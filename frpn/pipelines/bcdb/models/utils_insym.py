...
class ChainTokenFeaturePlus(nn.Module):
    ...
    def forward(
        ...
        for b in range(B):
            ...
            # Safety cap for memory: keep specials (CLS/GLOB/BLOCK0/BLOCK1),
            # and truncate repeat tokens to a fixed maximum.
            if self.max_chain_tokens is not None and self.max_chain_tokens > 0:
                special_len = 4
                max_total = special_len + int(self.max_chain_tokens)
                if tokens.size(0) > max_total:
                    tokens = torch.cat([tokens[:special_len], tokens[special_len:max_total]], dim=0)
                    blocks = torch.cat([blocks[:special_len], blocks[special_len:max_total]], dim=0)
